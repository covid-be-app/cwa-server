...
class SubmissionServiceConfigValidatorTest {
  ...
  void testWithValidResquestDataSizes(DataSize dataSize) {
    Errors errors = validateConfig(dataSize, "BE", getEmptyTekFieldDerivations());
  ...
  void testWithValidTrlFromDsos(Map<Integer, Integer> trlFromDsos) {
    ...
    Errors errors = validateConfig(SubmissionServiceConfigValidator.MAX_MAXIMUM_REQUEST_SIZE, "BE",
        tekFieldDerivations);
    assertThat(errors.hasErrors()).isFalse();
  }
