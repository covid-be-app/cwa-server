...
class S3RetentionPolicyTest {
  ...
  void shouldDeleteDiagnosisKeyHourFiles() {
    var toBeKept = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(1), "BE");
    var toBeDeleted = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(2), "BE");

    List<S3Object> mockResponse = this.s3ObjectsFromFilenames(Stream.concat(toBeDeleted.stream(), toBeKept.stream())
        .collect(Collectors.toUnmodifiableList()));

    when(objectStoreAccess.getObjectsWithPrefix(this.getDiagnosisKeyPrefix("BE"))).thenReturn(mockResponse);
  ...
  void shouldDeleteDiagnosisKeyHourFilesFromAllBuckets() {
    var toBeKeptBE = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(1), "BE");
    var toBeKeptEUR = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(1), "EUR");

    var toBeDeletedBE = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(2), "BE");
    var toBeDeletedEUR = generateDiagnosisKeyHourFilesForDay(getUtcDate().minusDays(2), "EUR");

    List<S3Object> mockResponseBE = this
        .s3ObjectsFromFilenames(Stream.concat(toBeDeletedBE.stream(), toBeKeptBE.stream())
            .collect(Collectors.toUnmodifiableList()));
    when(objectStoreAccess.getObjectsWithPrefix(this.getDiagnosisKeyPrefix("BE"))).thenReturn(mockResponseBE);
    ...
    toBeDeletedBE.forEach(f -> verify(objectStoreAccess, times(1)).deleteObjectsWithPrefix(f));
    toBeKeptBE.forEach(f -> verify(objectStoreAccess, never()).deleteObjectsWithPrefix(f));
  ...
  @Test
  void shouldRemoveDiagnosisKeyHourIndexFilesFromBEBucket() {
    var validHourFile = generateDiagnosisKeyHourIndex(getUtcDate().minusDays(1), "BE");
    var invalidHourFile = generateDiagnosisKeyHourIndex(getUtcDate().minusDays(2), "BE");
  ...
  void shouldRemoveDiagnosisKeyIndexFilesOlderThanCutoffDate() {
    var validHourFile = generateDiagnosisKeyHourIndex(getUtcDate().minusDays(1), "BE");
    var invalidHourFile1 = generateDiagnosisKeyHourIndex(getUtcDate().minusDays(2), "BE");
    var invalidHourFile2 = generateDiagnosisKeyHourIndex(getUtcDate().minusDays(3), "BE");
  ...
  void shouldDeleteOldDiagnosisKeyFiles() {
    ...
    List<S3Object> mockResponse = list(new S3Object("version/v1/configuration/country/BE/app_config"));
    mockResponse.addAll(
  ...
  void shouldDeleteTraceTimeWarningFilesOlderThanCutoffDate() {
    ...
    when(objectStoreAccess.getObjectsWithPrefix(this.getTraceTimeWarningPrefix("BE"))).thenReturn(mockResponse);
  ...
  private List<String> generateTraceTimeWarningFilenamesForRange(LocalDateTime startInclusive,
    ...
    return LongStream.range(TimeUnit.SECONDS.toHours(startInclusive.toEpochSecond(ZoneOffset.UTC)),
        ...
        .mapToObj(epochHour -> generateTraceTimeWarningFilename(epochHour, "BE"))
